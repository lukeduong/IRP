...
import math
import numpy as np
from pyfirmata import ArduinoMega, Arduino, SERVO, util
import time
...
# port2 = 'com5' 
#set up board layout
board1  = ArduinoMega(port1)
# board2  = Arduino(port2)
#-----------------------------------------------------------------------------#
#Define rotation functions for each servo
def rotateservo1(Servopin,angle):
        board1.digital[Servopin].write(angle)
# def rotateservo2(Servopin,angle):
        # board2.digital[Servopin].write(angle)       
#-----------------------------------------------------------------------------#
#Functions
...
#-----------------------------------------------------------------------------# 
      ...
      for i in range(2,(num_servos_per_row * num_servos_per_cols) + 2):
          ...
          board1.digital[i].mode = SERVO     #Assign pins as servos --> 2 is the first useable pin
          
          
          ServoPins.append(i)
          count+=1
          print(count)
      servo_pins_matrix = np.zeros((num_servos_per_row, num_servos_per_cols), dtype=object)
      for i in range(num_servos_per_row):
          for j in range(num_servos_per_cols):
              servo_pins_matrix[i][j] = ServoPins[j * num_cols_1_0_servo + i]
              count +=1
              if count == 20:
                  break
          if count == 20:                   
              break
      # servo_pins_matrix_1 = np.zeros((num_servos_per_row, num_servos_per_cols), dtype=object)
      # servo_pins_matrix_2 = np.zeros((num_servos_per_row, num_servos_per_cols), dtype=object)
      # count_1 = 0
      # count_2 = 0
      # for i in range(num_servos_per_row):
      #     for j in range(num_servos_per_cols):
      #         if count_1 < 20:
      #             servo_pins_matrix_1[i][j] = ServoPins[j * num_cols_1_0_servo + i]
      #             count_1 += 1
      #         else:
      #             servo_pins_matrix_2[i][j] = ServoPins[j * num_cols_1_0_servo + i]
      #             count_2 += 1
            ...
            for j in range(len(one_zero_servo[i])):
                ...
                if one_zero_servo[i][j] == 1:
                      ...
                      angle_matrix[i][j] = angle
                       # for k in range(len()):
                      rotateservo1(servo_pins_matrix[j][i],angle_matrix[i][j])
                      # rotateservo2(servo_pins_matrix_2[j][i],angle_matrix[i][j])
#-----------------------------------------------------------------------------#
