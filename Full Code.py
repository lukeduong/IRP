...
#connect Python to Arduino --> through serial port
port = 'com6' 
board  = Arduino(port)
...
#Reduce FPS
    ...
    # cv2.putText(gray, fps, (7, 70), , 3, (100, 255, 0), 3, cv2.LINE_AA)
#-----------------------------------------------------------------------------#
#FIRST WINDOW (image)
...
#-----------------------------------------------------------------------------#
    #Calculate distances between box centers and hand landmarks or hand connections
      ...
      for i, box_center in enumerate(box_centers_array):
              ...
              for j, connection in enumerate(hand_connection_list):   
                     x1, y1 = lmList[connection[0]][1], lmList[connection[0]][2]
                     x2, y2 = lmList[connection[1]][1], lmList[connection[1]][2]
                     x3, y3 = box_center[0], box_center[1]
                     
                     # cv2.circle(image,(x1, y1), 5, (255,0,0), -1)            # Check if for loop correctly selecting the landmark coordinates for each hand connection            
                     # cv2.circle(image,(x2, y2), 5, (255,0,0), -1)
                     
                   # Convert hand connection line to equation of a line 
                   #  Compute the slope and intercept of the line between the two landmarks
                     if x2 != x1:
                         m = (y2 - y1) / (x2 - x1)
                         b = y1 - (m * x1)
                     else:
                         m = float('inf')
                         b = x1  # vertical line, so intercept is x-coordinate
                     # Convert the point-slope form to the form Ax + By + C = 0
                     A = (-1) * m
                     B = 1
                     C = (-1) * b
                     A_round = round(A,3)
                     C_round = round(C,3)
                     #Save equations of lines in dictionary with the respective landmarks
                     EoL = f"{A_round}x + {B}y + {C_round} = 0"
                     hand_connection_line_eq[EoL] = (lmList[connection[0]],lmList[connection[1]])
                     
                     #Use distance_point_to_line function to calculate distance and save as list
                     connection_dist1 = distance_point_to_line(x3, y3, A, B, C)
                     connection_dist_dictionary_key = f"{j}" 
                     connection_dist_dictionary[connection_dist_dictionary_key] = (connection_dist1, (box_center[0], box_center[1]), (x1,y1), (x2,y2))
                     connection_dist.append(connection_dist1)
                     distances.append(connection_dist1)
                     key = f"Box {i+1} to Connection {j} (lm {j} to lm {j+1})"
                     distances_dictionary[key] = (connection_dist1, (x1,y1), (x2,y2))                        # distances --> gives a dictionary of the distances perpedenicularly from lm or connection
...
#-----------------------------------------------------------------------------#      
      ...
      for i in range(0 , len(distances), 42):
          full_distances_list.append(distances[i:i+42])
...
#-----------------------------------------------------------------------------# 
      ...
      ServoPins = []
      for i in range(2,(num_servos_per_row * num_servos_per_cols) + 2):
          board.digital[i].mode = SERVO     #Assign pins as servos --> 2 is the first useable pin
          ServoPins.append(i)


      servos = [[False, 0] for _ in range(num_servos_per_row * num_servos_per_cols)]
      angle_matrix = np.zeros(matrix_grid, dtype=float)
      # Loop through the one_zero_servo matrix
      for i in range(num_servos_per_row):
          for j in range(num_servos_per_cols):
              if one_zero_servo[i][j] == 1:
                  # Turn on the corresponding LED
                  servos[i * num_servos_per_cols + j][0]                  #ON
                  angle = 180 - ((180/threshold) * min_dist_servo[i][j])
                  angle_matrix[i][j] = angle
                  rotateservo(ServoPins[i*j],angle)
              else:
                  # Turn off the corresponding LED
                  servos[i * num_servos_per_cols + j][0]                   #Off 
