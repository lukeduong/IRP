...
top_label = [f"{i}" for i in range(1,grid_dimensions[0] + 2)]
side_label = [f"{i}" for i in range(1,grid_dimensions[1] + 2)]
    ...
    # Calculate box center points and draw centre point
      for y in range(grid_dimensions[1]+1):
          for x in range(grid_dimensions[0]+1):
              ...
              # cv2.circle(image3,(box_x_pos, box_y_pos ), 18, (255,0,0), 2) 
              # cv2.circle(image,(box_x_pos, box_y_pos ), 18, (255,0,0), 2) 
...
#-----------------------------------------------------------------------------#
    #Calculate distances between box centers and hand landmarks or hand connections
      ...
      thresholdx = int(w/(2 * grid_array[0]))
      thresholdy = int(h/(2 * grid_array[1]))
...
#-----------------------------------------------------------------------------#      
      ...
      min_dist_servo = np.fliplr(min_dist_servo)
      one_zero_servo = np.fliplr(one_zero_servo)
...
#-----------------------------------------------------------------------------# 
      ...
      fig, ax = plt.subplots()  
      im, cbar = heatmap(one_zero_servo, side_label, top_label, ax=ax,
                   cmap="Blues", cbarlabel="Servo actuation")
